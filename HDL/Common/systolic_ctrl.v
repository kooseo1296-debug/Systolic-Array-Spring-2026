...
module systolic_ctrl (
    ...
    output [`BIT_SPSRAM-1:0] Out_addra_WeightSRAM,
...
reg FinishFlag;
reg [`BIT_SPSRAM-1:0] Out_addra_WeightSRAM_0;
assign Out_addra_WeightSRAM = Out_addra_WeightSRAM_0 + Offset; //Load weight address with offset imposed
...
always @(posedge CLK) begin
    if(~RST) begin
        ...
        Out_addra_WeightSRAM_0 <= {(`BIT_SPSRAM){1'b0}};
        ...
        if(Instr_In[(`BIT_DATA+`BIT_PARAM)+:(`BIT_OPCODE+1)] == {`OPVALID, `OPCODE_LDSRAM}) begin
            ...
            Out_addra_WeightSRAM_0 <= LoadWeightAddress; 
                ...
                if(~RunState) begin //Weight Load Mode
                    ...
                    if(RowCount <`PE_ROW) begin
                        ...
                        for(j=0;j<`PE_COL;j=j+1) begin
                            ...
                            Out_addra_WeightSRAM_0 <= (IC*OC_Count[3+:(`BIT_DATA-3)] + IC_Count + RowCount); //Weight SRAM's Address Representation Applied
                        end
                    end
                    ...
                    if (S_Count < SC) begin //Load Input
                        ...
                        if(OC_Count < OC) begin
                            ...
                            Out_addrP_SysDffq[0+:`BIT_ADDR] <= (OC_Count[`BIT_DATA-1:3]*SC + S_Count);
                            Out_addrb_PsumSRAM[0+:`BIT_DPSRAM] <= (OC_Count[`BIT_DATA-1:3]*SC + S_Count);
                        end
        ...
        WriteBackTimer <= 3'd0;
        WriteBackFilter <= `PE_COL'd0;
        WriteBackFilter_1 <= `PE_COL'd0;
        WriteBackFilter_2 <= `PE_COL'd0;
        WriteBackFilter_3 <= `PE_COL'd0;
        WriteBackFilter_4 <= `PE_COL'd0;
        Out_enable_PsumLoader <= `PE_COL'd0;
    end
end
